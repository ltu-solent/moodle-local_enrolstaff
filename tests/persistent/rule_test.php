...
final class rule_test extends \advanced_testcase {
     ...
     *
     * Notify is only used if sendas is not nonotification. The options allow for a set list, plus user defined emails,
     * but they must exist as users in Moodle. Otherwise, roles are used.
     *
     * @param string $sendas
     * @param string $notify
     * @param string $backupnotifyemail
     * @param string $registryemail
     * @param bool $success
     * @param string $validationstring
     * @return void
     * @dataProvider validate_notify_provider
     */
    public function test_validate_notify($sendas, $notify, $backupnotifyemail, $registryemail, $success, $validationstring): void {
        $this->resetAfterTest();
        $this->create_roles();
        ...
        if (!empty($backupnotifyemail)) {
            $this->getDataGenerator()->create_user([
                'email' => $backupnotifyemail,
            ]);
        }
        set_config('availablebackupnotifyemails', $backupnotifyemail, 'local_enrolstaff');
        if (!empty($registryemail)) {
            $this->getDataGenerator()->create_user([
                'email' => $registryemail,
            ]);
        }
        set_config('availableregistryemails', $registryemail, 'local_enrolstaff');
        /** @var \local_enrolstaff_generator $esdg */
        $esdg = $this->getDataGenerator()->get_plugin_generator('local_enrolstaff');
        $notifyvalues = api::clean_csv($notify);
        // Swap rolenames for ids.
        $recordnotifyvalues = [];
        foreach ($notifyvalues as $value) {
            $parts = explode(':', $value);
            if ($parts[0] === 'r') {
                $rolename = $parts[1];
                if (!isset($this->roles[$rolename])) {
                    // Invalid role name.
                    $recordnotifyvalues[] = 'r:' . $parts[1];
                } else {
                    $recordnotifyvalues[] = 'r:' . $this->roles[$rolename];
                }
            } else {
                // Emails don't always get prefixed with 'e:'.
                if (count($parts) == 1) {
                    $recordnotifyvalues[] = 'e:' . $parts[0];
                } else {
                    $recordnotifyvalues[] = 'e:' . $parts[1];
                }
            }
        }
        ...
        $params = [
            ...
            'value' => implode(',', $recordnotifyvalues),
        ];

        try {
            $rule = $esdg->create_rule([
                'sendas' => $sendas,
                'notify' => $recordnotifyvalues,
                'roleids' => [$this->roles['tutor']],
            ]);
            if ($success) {
                $this->assertEquals($recordnotifyvalues, $rule->get('notify'));
                return;
            }
            $this->fail('Expected exception not thrown ' . implode(',', $rule->get('notify')));
        } catch (invalid_persistent_exception $e) {
            if (!$success) {
                $this->assertStringContainsString(
                    get_string($validationstring, 'local_enrolstaff', $params),
                    $e->getMessage()
                );
                return;
            }
            mtrace($sendas . ' - ' . implode(',', $recordnotifyvalues));
            $this->fail('Exception thrown unexpectedly ' . $e->getMessage());
        }
    }

    /**
     * Validate notify provider
     *
     * @return array
     */
    public static function validate_notify_provider(): array {
        return [
            'valid notify - nonotification' => [
                'sendas' => 'nonotification',
                'notify' => '',
                'backupnotifyemail' => 'backupnotify@example.com',
                'registryemail' => 'registry@example.com',
                'success' => true,
                'validationstring' => '',
            ],
            'notification with no notify items - fail' => [
                'sendas' => 'notification',
                'notify' => '',
                'backupnotifyemail' => '',
                'registryemail' => 'registry@example.com',
                'success' => false,
                'validationstring' => 'invalidnotifysendas',
            ],
            'notification with invalid roles - fail' => [
                'sendas' => 'notification',
                'notify' => 'r:invalidrole',
                'backupnotifyemail' => '',
                'registryemail' => 'registry@example.com',
                'success' => false,
                'validationstring' => 'invalidfield',
            ],
            'notification with valid role' => [
                'sendas' => 'notification',
                'notify' => 'r:moduleleader',
                'backupnotifyemail' => '',
                'registryemail' => 'registry@example.com',
                'success' => true,
                'validationstring' => '',
            ],
            'notification with unavailable role - fail' => [
                'sendas' => 'notification',
                'notify' => 'r:tutor',
                'backupnotifyemail' => '',
                'registryemail' => 'registry@example.com',
            ...
            'notification with valid but non-existent email - fail' => [
                'sendas' => 'notification',
                'notify' => 'e:validemail@example.com',
                'backupnotifyemail' => '',
                'registryemail' => 'registry@example.com',
                'success' => false,
                'validationstring' => 'invalidfield',
            ],
            'notification with valid existing email' => [
                'sendas' => 'notification',
                'notify' => 'backupnotify@example.com',
                'backupnotifyemail' => 'backupnotify@example.com',
                'registryemail' => 'registry@example.com',
                'success' => true,
                'validationstring' => '',
